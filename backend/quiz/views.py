...
CLOUDFLARE_API_URL = f"https://api.cloudflare.com/client/v4/accounts/{ACCOUNT_ID}/images/v1"
...
def signup_mongo(request):
    ...
    security_question_key = (data.get("securityQuestion") or "").strip()
    security_answer = (data.get("securityAnswer") or "").strip()
    normalized_security_answer = normalize_security_answer(security_answer)
    ...
    if security_question_key not in SECURITY_QUESTIONS:
        return JsonResponse({"error": "Please select a security question."}, status=400)
    if not normalized_security_answer:
        return JsonResponse({"error": "Security answer is required."}, status=400)
    ...
    password_hash = make_password(password)
    security_answer_hash = make_password(normalized_security_answer)
    user_doc = {
        "email": email,
        "username": email,
        "password_hash": password_hash,
        "display_name": display_name,
        "security_question": security_question_key,
...
def forgot_password(request):
    ...
    if not user:
        return JsonResponse({"error": "We couldn't find an account with that email."}, status=404)

    stored_question = user.get("security_question")
    if not stored_question or stored_question != security_question_key:
        return JsonResponse({"error": "Security question does not match our records."}, status=400)

    stored_answer_hash = user.get("security_answer_hash")
    if not stored_answer_hash or not check_password(normalized_security_answer, stored_answer_hash):
        return JsonResponse({"error": "Security answer is incorrect."}, status=401)
...
@csrf_exempt
def admin_search_image(request):
    ...
    # regex search (case-insensitive, partial match) on filename with pagination
    results = list(collection.find(
        {"filename": {"$regex": query, "$options": "i"}},
        {"_id": 0, "filename": 1, "created_at": 1, "source_url": 1}  # include source_url
    ).skip(skip).limit(page_size))

    # ensure every result has a source_url for the frontend
    for r in results:
        if "source_url" not in r or not r["source_url"]:
            r["source_url"] = f"{PUBLIC_URL_BASE}{r.get('filename', '')}"

    print(f"[DEBUG] Search query: '{query}' page {page} returned {len(results)} of {total_count} images")

    return JsonResponse({"items": results, "total": total_count})

@csrf_exempt
def admin_delete_image(request, filename):
    if request.method != "DELETE":
        return JsonResponse({"error": "Invalid request"}, status=400)
    ...
    # Find the image in your images collection
    item = collection.find_one({"filename": filename})
    if not item:
    ...
    wardrobe_collection.delete_many({"filename": filename})
...
def search_images(request):
    ...
    items = list(images_db.find({"name": {"$regex": query, "$options": "i"}}, {"_id": 0}))
...
def delete_image(request):
    ...
    if not name:
        return JsonResponse({"detail": "Image name is required"}, status=400)

    # 1. Delete from MongoDB images collection
    ...
    cf_resp = requests.delete(f"{CLOUDFLARE_API_URL}/names/{name}", headers=headers)
    if cf_resp.status_code not in [200, 204]:
        return JsonResponse({"detail": "Failed to delete from Cloudflare"}, status=500)

    return JsonResponse({"detail": "Image deleted successfully"})
...
def delete_user(request):
    ...
    try:
        ...
        # Replace this with your MongoDB deletion logic
...
def upload_image(request):
    ...
    try:
        # Upload to R2 (like in generate)
        r2_url = upload_to_r2(filename, file.read())  # make sure upload_to_r2 accepts bytes
        ...
        collection.insert_one({
            ...
            "is_ai": False,  # admin uploads are not AI-generated
            "created_at": datetime.utcnow(),
        })

        return JsonResponse({"message": "Image uploaded", "url": r2_url})
    
    except Exception as e:
        print(f"[DEBUG] Failed admin upload: {e}")
        return JsonResponse({"error": "Upload failed"}, status=500)


@csrf_exempt
def upload_instantoutfit(request):
    """Admin endpoint: upload an instant outfit image and save into instantoutfit collection.
    Accepts: image (file), filename (optional), tag (single value from allowed set).
    Also sets `vibe` equal to the tag.
    """
    if request.method != "POST":
        return JsonResponse({"error": "Invalid request"}, status=400)
    ...
    filename = request.POST.get("filename", file.name)
    tag_raw = (request.POST.get("tag") or "").strip()
    if not tag_raw:
        return JsonResponse({"error": "Tag is required"}, status=400)

    allowed = {"sunny", "cloudy", "cold", "casual", "work", "date"}
    tag = tag_raw.strip().lower()
    if tag not in allowed:
        return JsonResponse({"error": "Invalid tag value"}, status=400)

    try:
        r2_url = upload_to_r2(filename, file.read())
        if not r2_url:
            return JsonResponse({"error": "Upload failed"}, status=500)

        # Insert into instant collection
        instant_doc = {
            "filename": filename,
            "tags": [tag],
            "vibe": tag,
            "source_url": r2_url,
            "images": {"full": r2_url, "thumbnail": r2_url},
            "uploaded_by": decoded.get("user_id"),
            "is_ai": False,
            "created_at": datetime.utcnow(),
        }
        instant_collection.insert_one(instant_doc)

        return JsonResponse({"message": "Instant outfit uploaded", "url": r2_url})
    except Exception as e:
        print(f"Failed instantoutfit upload: {e}")
        return JsonResponse({"error": "Upload failed"}, status=500)


@csrf_exempt
def admin_search_instantoutfit(request):
    """Search instant outfits with pagination, similar to admin_search_image"""
    query = request.GET.get("q", "").strip()
    page = int(request.GET.get("page", 1))
    page_size = int(request.GET.get("page_size", 10))
    ...
    # Count total results for pagination
    total_count = instant_collection.count_documents(
        {"filename": {"$regex": query, "$options": "i"}}
    )

    # regex search (case-insensitive, partial match) on filename with pagination
    results = list(instant_collection.find(
        {"filename": {"$regex": query, "$options": "i"}},
        {"_id": 0, "filename": 1, "created_at": 1, "source_url": 1, "vibe": 1, "tags": 1}
    ).skip(skip).limit(page_size))
...
@csrf_exempt
def admin_delete_instantoutfit(request, filename):
    """Delete instant outfit from DB and R2 storage"""
    if request.method != "DELETE":
        return JsonResponse({"error": "Invalid request"}, status=400)
    ...
    try:
        # Delete from MongoDB
        result = instant_collection.delete_one({"filename": filename})
        if result.deleted_count == 0:
            return JsonResponse({"error": "Instant outfit not found"}, status=404)

        # Delete from R2
        try:
            s3.delete_object(Bucket=BUCKET, Key=filename)
        except Exception as e:
            print(f"Warning: Could not delete {filename} from R2: {e}")
...
def recommend(request):
    ...
    # Stratified sampling approach for better distribution
    def fetch_with_stratified_sampling():
        """
        Fetch images using stratified sampling to ensure better tag distribution
        """
        expanded_queries = expand_queries(base_tags)
        
        # Step 1: Try exact match first (all required tags present)
        exact_match_conditions = []
        if base_tags:
            exact_match_conditions.append({"tags": {"$all": base_tags}})
        if preferred_weather:
            exact_match_conditions.append({"tags": preferred_weather})
        
        if exact_match_conditions:
            if len(exact_match_conditions) == 1:
                exact_query = exact_match_conditions[0]
            else:
                exact_query = {"$and": exact_match_conditions}
            
            # Use random sampling for better distribution
            try:
                exact_docs = list(collection.aggregate([
                    {"$match": exact_query},
                    {"$sample": {"size": min(image_count * 2, 50)}},
                    {"$sort": {"created_at": -1}}
                ]))
            except:
                # Fallback if $sample fails (e.g., small collection)
                exact_docs = list(collection.find(exact_query).sort("created_at", -1).limit(image_count * 2))
        ...
        # Step 2: If not enough exact matches, try partial matches with expanded queries
        if len(response_images) < image_count:
            partial_conditions = []
            if expanded_queries:
                partial_conditions.append({"tags": {"$in": expanded_queries}})
            if preferred_weather:
                partial_conditions.append({"tags": preferred_weather})
            
            # Exclude already found documents
            if seen_names:
                partial_conditions.append({"filename": {"$nin": list(seen_names)}})
            
            if partial_conditions:
                if len(partial_conditions) == 1:
                    partial_query = partial_conditions[0]
                else:
                    partial_query = {"$and": partial_conditions}
                
                remaining_needed = image_count - len(response_images)
                try:
                    partial_docs = list(collection.aggregate([
                        {"$match": partial_query},
                        {"$sample": {"size": min(remaining_needed * 4, 100)}},
                        {"$sort": {"created_at": -1}}
                    ]))
                except:
                    # Fallback if $sample fails
                    partial_docs = list(collection.find(partial_query).sort("created_at", -1).limit(remaining_needed * 4))
        ...
        # Step 3: Final fallback - any images that match individual tags
        if len(response_images) < image_count:
            nonlocal unique_exhausted
            unique_exhausted = True
            
            fallback_conditions = []
            if base_tags:
                # Look for images with any of the individual tags
                for tag in base_tags:
                    fallback_conditions.append({"tags": tag})
            if preferred_weather:
                fallback_conditions.append({"tags": preferred_weather})
            
            # Exclude already found documents
            if seen_names:
                exclude_condition = {"filename": {"$nin": list(seen_names)}}
            else:
                exclude_condition = {}
            ...
            if fallback_conditions:
                fallback_query = {
                    "$and": [
                        ...
                        exclude_condition
                    ] if exclude_condition else [{"$or": fallback_conditions}]
                } if exclude_condition else {"$or": fallback_conditions}
                
                remaining_needed = image_count - len(response_images)
                try:
                    fallback_docs = list(collection.aggregate([
                        {"$match": fallback_query},
                        {"$sample": {"size": min(remaining_needed * 6, 150)}},
                        {"$sort": {"created_at": -1}}
                    ]))
                except:
                    fallback_docs = list(collection.find(fallback_query).sort("created_at", -1).limit(remaining_needed * 6))
            ...
            # Step 4: Absolute last resort - any images
            if len(response_images) < image_count:
                try:
                    any_docs = list(collection.aggregate([
                        {"$sample": {"size": min((image_count - len(response_images)) * 2, 50)}}
                    ]))
                except:
                    any_docs = list(collection.find({}).sort("created_at", -1).limit((image_count - len(response_images)) * 2))
    ...
    # Execute the stratified sampling
...
def export_users(request):
    """
    Export all registered users as an Excel workbook.
    """
    permission_error = ensure_admin(request)
    if permission_error:
        return permission_error
    ...
    sheet.append(["Username", "Email", "Role", "Created At"])
    ...
    filename = f"registered_users_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.xlsx"
    response = HttpResponse(
        stream.getvalue(),
        content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    )
