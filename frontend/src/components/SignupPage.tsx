...
export default function SignupPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!formValues.securityAnswer.trim()) {
    ...
    try {
      const response = await fetch(apiUrl("/api/signup_mongo/"), {
        ...
        body: JSON.stringify({
          ...
          securityAnswer: formValues.securityAnswer.trim(),
        }),
      });
    ...
    <section className="flex min-h-[calc(100vh-3.5rem)] items-center justify-center bg-gradient-to-br from-amber-200 via-fuchsia-200 to-indigo-200 px-4 text-zinc-900">
      <div className="w-full max-w-lg rounded-3xl border border-white/60 bg-white/80 p-8 shadow-2xl backdrop-blur">
        ...
        <form className="space-y-5" onSubmit={handleSubmit} noValidate>
          ...
          <label className="block text-left text-sm font-medium text-zinc-700" htmlFor="securityQuestion">
            Security question
            <select
              id="securityQuestion"
              ...
              onChange={handleChange}
              className="select-light-options mt-2 w-full rounded-2xl border border-white/80 bg-white px-4 py-3 text-base text-zinc-900 shadow-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-400/60"
            >
          ...
          <label className="block text-left text-sm font-medium text-zinc-700" htmlFor="securityAnswer">
            Answer
            <input
              id="securityAnswer"
              name="securityAnswer"
